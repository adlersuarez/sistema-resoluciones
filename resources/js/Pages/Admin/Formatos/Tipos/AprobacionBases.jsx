...
localStorage.setItem("fecha_carta_visto", fecha_carta_visto)
localStorage.setItem("num_carta_visto", num_carta_visto)

localStorage.setItem("muestra_fecha", muestra_fecha)

const AprobacionBases = ({ auth ,facultad }) => {
    ...
    function agregar_visto() {
        const visto_datos = `La Resolución de Consejo Universitario N° ${numeroResolucion(data.num_res_visto)}-2023-CU de fecha ${fecha(data.fecha_res_visto)}, Carta N° ${numeroResolucion(data.num_carta_visto)} de fecha ${fecha(data.fecha_carta_visto)}, Oficio Digital N° ${numeroResolucion(data.num_ofi_visto)}-R-UPLA de fecha ${fecha(data.fecha_ofi_visto)} y acuerdo de Consejo Universitario en sesión ordinaria de fecha ${fecha(localStorage.getItem('fecha_resolucion'))}, respectivamente; y,`
    ...
    function agregar_considerando() {
        listaConsiderando.push({
            descripcion: "El Artículo 23°, inc. v) y w) del Estatuto de la Universidad Peruana Los Andes señala que, el Consejo Universitario tiene atribuciones para conocer y resolver todos los demás asuntos que no están encomendados a las autoridades universitarias; y otras que señale el Estatuto, el Reglamento de Organización y Funciones (ROF) y demás reglamentos de la Universidad; y en el artículo 264°, dispone que la reinversión de excedentes en la Universidad se aplica en infraestructura, equipamiento para fines educativos, investigación e innovación en ciencia y tecnología, capacitación y actualización de docentes, proyección social, apoyo al deporte de alta calificación, programas deportivos y bienestar universitario; así como la concesión de becas, conforme a la normatividad aplicable;",
            id: "1",
            nombre: "Estatuto",
            tipo: "General"
        })
        ...
        listaConsiderando.push({
            descripcion: "La Resolución N° 1315-2021-CU de fecha 26.08.2021 mediante la cual el Consejo Universitario, aprueba el Reglamento de Adquisición de Bienes y Contratación de Servicios de la Universidad Peruana Los Andes;",
            id: "2",
            nombre: "Resolución",
            tipo: "General"
        })
        ...
        listaConsiderando.push({
            descripcion: `La Resolución de Consejo Universitario N° ${numeroResolucion(localStorage.getItem('num_res_visto'))}-2023-CU de fecha ${fecha(localStorage.getItem('fecha_res_visto'))} mediante la cual, conforma la comisión encargada de la elaboración de bases, convocatoria y selección del Supervisor para la obra "${localStorage.getItem('nom_obra')}";`,
            id: "3",
            nombre: "Miembros",
            tipo: "General"
        })
        ...
        listaConsiderando.push({
            descripcion: `El Oficio Digital N° ${numeroResolucion(localStorage.getItem('num_ofi_visto'))}-2023-R-UPLA de fecha ${fecha(localStorage.getItem('fecha_ofi_visto'))} emitido por el señor Rector mediante el cual remite el expediente para ser puesto a consideración de Consejo Universitario;`,
                                ...
                                <div className=''>
                                    ...
                                    <div className='flex my-2'>
                                        ...
                                        <input
                                            ...
                                            min="1" max="999"
                                            className="px-4 py-2 text-gray-500 h-8 my-auto"
                                            defaultValue={localStorage.getItem('num_carta_visto')}
                                            onChange={(e) => {
                                                setData('num_carta_visto', e.target.value);
                                                localStorage.setItem("num_carta_visto", e.target.value);
                                            }}
                                            required
                                        />
                                        ...
                                        <input
                                            type="date"
                                            className="text-gray-500 h-8 my-auto w-36"
                                            defaultValue={localStorage.getItem('fecha_carta_visto')}
                                            onChange={(e) => {
                                                setData('fecha_carta_visto', e.target.value);
                                                localStorage.setItem("fecha_carta_visto", e.target.value);
                                            }}
                                            required
                                        />
                                    </div>
...
export default AprobacionBases