...
const CalendarioAcademicoGeneral = ({ auth }) => {
    ...
    function agregar_visto() {
        const visto_datos = `El Oficio N° ${numeroResolucion(data.num_res_visto)}-E-2023-ORYM-UPLA de fecha ${fecha(data.fecha_res_visto)}, Proveído N° ${numeroResolucion(data.num_ofi_visto)}-2023-VRACD-UPLA de fecha ${fecha(data.fecha_ofi_visto)}, Oficio Digital N° ${numeroResolucion(data.num_ofi_visto_2)}-2023-R-UPLA de fecha ${fecha(data.fecha_ofi_visto_2)} y acuerdo de Consejo Universitario en sesión ordinaria de fecha ${fecha(localStorage.getItem('fecha_resolucion'))}, respectivamente; y,`
    ...
    function agregar_considerando() {
        ...
        listaConsiderando.push({
            descripcion: "El artículo 23°, inc. w) del Estatuto de la Universidad Peruana Los Andes, dispone que el Consejo Universitario tiene atribución para resolver otras que señale el Estatuto, el Reglamento de Organización y Funciones (ROF) y demás reglamentos de la Universidad;",
            id: "2",
            nombre: "Resolución",
            tipo: "General"
        })
    ...
    return (
        <Navbar auth={auth}>
            ...
            <div className='w-full flex justify-between mt-10 mb-5'>
                <TitlePages texto={'Registrar Resolución de Calendario académico a nivel general'} icono={faFileWord} />
                ...
                <div className='bg-white shadow-sm sm:rounded-lg w-6/12 p-4 ml-4 overflow-hidden'>
                    <div className='h-[800px] border-black border-[1px] p-2 overflow-x-hidden max-h-[745px] max-[745px]:overflow-scroll'>
                        <div className='my-2 text-center text-xl'>
                            <strong>RESOLUCIÓN DE CALENDARIO ACADÉMICO A NIVEL GENERAL
                                {
...
export default CalendarioAcademicoGeneral