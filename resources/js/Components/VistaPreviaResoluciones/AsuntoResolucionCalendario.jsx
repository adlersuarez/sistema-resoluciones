...
import TablaVistaPrevia from '../Tablas/VistaPrevia/TablaVistaPrevia';

export default function AsuntoResolucionCalendario({ listaAsuntos }) {
        ...
        <>
            {
                listaAsuntos.map((asunto, index) => {
                    //para el primer articulo añadir la tabla
                    if (index == 0) {
                        return (
                            <div className='grid grid-cols-12 mt-2' key={asunto.cod}>
                               ...
                               <div className='col-span-9 mx-2 flex'>
                                <TablaVistaPrevia datos={JSON.parse(localStorage.getItem('actividadesFormulario'))} />
                               </div>
                            </div>
